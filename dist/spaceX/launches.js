    ...
    getLaunches(start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const startDateAsMoment = moment_1.default(start, 'YYYY');
                const endDateAsMoment = moment_1.default(end, 'YYYY');
                // Check that the date string is valid, and that the date is before or equal to today's date
                // moment() creates a moment with today's date
                if (!startDateAsMoment.isValid() || !startDateAsMoment.isSameOrBefore(moment_1.default())
                    || !endDateAsMoment.isValid() || !endDateAsMoment.isSameOrBefore(moment_1.default())) {
                    return [{ error: 'invalid start or end year' }];
                }
                // Get the parsed request
                const parsed = yield this.requestLaunchesByYear(start);
                // Maps and creates the return list
                return this.filterFields(parsed);
            }
            catch (e) {
